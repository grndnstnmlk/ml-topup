...
// GET /api/admin/my-ip - cek IP keluar server untuk Whitelist Digiflazz
...
// GET /api/admin/orders - daftar order terbaru
...
router.post('/orders/:order_id/retry', async (req, res) => {
  try {
    ...
    console.error('Error admin retry:', err.message);
    res.status(500).json({ error: 'Gagal kirim ulang: ' + err.message });
  }
});

// POST /api/admin/orders/:order_id/mark-paid - konfirmasi manual
router.post('/orders/:order_id/mark-paid', async (req, res) => {
  try {
    ...
    console.error('Error mark-paid:', err.message);
    res.status(500).json({ error: 'Gagal menandai lunas: ' + err.message });
  }
});
...
router.post('/orders/:order_id/mark-delivered', async (req, res) => {
  try {
    ...
    console.error('Error mark-delivered:', err.message);
    res.status(500).json({ error: 'Gagal menandai terkirim: ' + err.message });
  }
});
  ...
  try {
    ...
    await db.run(
      `UPDATE orders 
       ...
       WHERE order_id = ?`,
      [newStatus, newDeliveryStatus, newSn, newMsg, orderId]
    );

    res.json({ ok: true, order_id: orderId });
  } catch (err) {
    console.error('Error update status:', err.message);
    res.status(500).json({ error: 'Gagal update status: ' + err.message });
  }
});
